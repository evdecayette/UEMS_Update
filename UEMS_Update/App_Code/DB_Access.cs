...
public class DB_Access : IDisposable
{
    ...
    public bool EtudiantDejaInscrit(string PersonneID, string NumeroCours, SqlConnection sqlConn)
    {
        ...
        try
        {
            ...
            SqlCmd = new SqlCommand(sSql, sqlConn);
            SqlDataReader dtTemp = SqlCmd.ExecuteReader();
    ...
    public void FacturerPourObligation(string sObligationLabel, string sPersonneID, SqlConnection sqlConn)
    {
        ...
        try
        {
            SqlCmd = new SqlCommand
            {
                ...
                // Facturer
                CommandText = String.Format("INSERT INTO MontantsDus (PersonneID, CodeObligation, Montant) SELECT '{0}', '{1}', Montant " +
    ...
    public void FacturerPourObligation(string sObligationLabel, string sPersonneID, String sMontant, SqlConnection sqlConn)
    {
        ...
        try
        {
            SqlCmd = new SqlCommand
            {
                ...
                // Facturer
                CommandText = String.Format("INSERT INTO MontantsDus (PersonneID, CodeObligation, Montant) VALUES ('{0}', '{1}', {2}) ",
    ...
    public double GetNoteDePassage(int iCoursOffertID, SqlConnection sqlConn)
    {
        double dtRetVal = 0;
        //SqlConnection sqlConn = null;
        SqlCommand SqlCmd;
        ...
        try
        {
            SqlCmd = new SqlCommand(string.Format("SELECT NotePassage FROM CoursOfferts WHERE CoursOffertID = {0}", iCoursOffertID), sqlConn);
            dtReader = SqlCmd.ExecuteReader();

            if (dtReader.Read())
            {
                dtRetVal = double.Parse(dtReader["NotePassage"].ToString());
                dtReader.Close();
                dtReader = null;
            }
        }
    ...
    /// <summary>
    /// List of students who already took and and passed this class
    /// </summary>
    /// <param name="NumeroCours"></param>
    ...
    public Hashtable GetEtudiantsAlreadyTookThisCourse(String sNumeroCours, SqlConnection sqlConn)
    {
        Hashtable ht = new Hashtable();

        // Loop through all records - SELECT PersonneID FROM CoursPris WHERE NumeroCours = 'GES100' AND NoteSurCent >= NotePassage
        try
        {
            String hashSQL = String.Format("SELECT PersonneID FROM CoursPris WHERE NumeroCours = '{0}' AND NoteSurCent >= NotePassage", sNumeroCours);
    ...
    /// <returns></returns>
    public int GetDisciplineIDFromName(string DisciplineNom, SqlConnection sqlConn)
    {
        int iReturnedValue = 0;
        try
        {
            string sSql = String.Format("SELECT DisciplineID FROM Disciplines WHERE DisciplineNom = '{0}'", DisciplineNom);
            SqlDataReader dt = GetDataReader(sSql, sqlConn);
            if (dt != null)
            {
                if (dt.Read())
                {
                    iReturnedValue = int.Parse(dt["DisciplineID"].ToString());
                }
                dt.Close();
                dt = null;
            }
    ...
    /// <param name="i"></param>
    /// <param name="s"></param>
    public bool PrepareTableDeControle(SqlConnection sqlConn)
    {
        bool retVal = false;
        try
        {
    ...
    public double GetTauxGourdesDollarUSFromDepense(int DepenseID, SqlConnection sqlConn)
    {
        ...
        try
        {
            ...
            {
                Connection = sqlConn,
                CommandText = "SELECT Taux_Interet FROM Depenses WHERE DepenseID = " + DepenseID
            };
    ...
    public HashSet<Notation> FillNotations(SqlConnection sqlConn)
    {
        ...
        if (dtTemp != null)
            while (dtTemp.Read())
            {
                ...
                {
                    Min = int.Parse(dtTemp["Min"].ToString()),
                    Max = int.Parse(dtTemp["Max"].ToString()),
                    Lettre = dtTemp["Lettre"].ToString(),
                    NotationLiterale = dtTemp["NotationLiterale"].ToString(),
                    Moyenne = float.Parse(dtTemp["Moyenne"].ToString())
                };
                hs.Add(notation);
            }
    ...
    public bool SendMailToUser(string sEmailTo, string sBodyMessage, string sSubject, List<Attachment> AttachmentPaths)
    {
        ...
        try
        {
            ...
            smtp = new SmtpClient(sHost, iPort)
            {
                ...
                Credentials = new NetworkCredential(sReturnAddressEmail, sPassWord),
                EnableSsl = true
            };
